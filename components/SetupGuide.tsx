  ...
  try {
    ...
    data.forEach((row, index) => {
      ...
      if (paymentDateValue instanceof Date) {
        // Format date using spreadsheet's timezone to avoid UTC conversion issues
...
function doPost(e) {
  try {
    // Data is now sent via FormData, so we access it from e.parameter
    ...
    // e.parameter values are strings, so we need to parse rowIndex to an integer for getRange
      ...
      <div className="space-y-6 text-gray-700">
        ...
        <div>
          ...
          <ul className="list-disc list-inside space-y-1 pl-4">
            ...
            <li>Copy the code below and paste it into the editor. <span className="font-bold text-red-600">(Note: The `doGet` function has been updated to fix date issues.)</span></li>
          </ul>
        ...
        <div>
          ...
          <ul className="list-disc list-inside space-y-1 pl-4">
            ...
            <li>Copy the generated <span className="font-semibold">Web app URL</span>.</li>
          </ul>
        </div>

        <div>
          <h2 className="text-xl font-semibold mb-2">Step 4: Update the App Code</h2>
          <p>Open the file <code className="bg-gray-200 px-1 rounded">services/googleSheetsService.ts</code> in your project and replace the placeholder with the URL you just copied.</p>
          ...
          <p className="mt-2">After updating the URL, save the file and refresh this page.</p>
        </div>
      </div>
    </div>
  );
